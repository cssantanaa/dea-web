...
import { CriarEstabelecimento, AtualizarEstabelecimento } from '../schema/estabelecimento.schema'
import { auditoriaService } from '../services/auditoria.service'

export const estabelecimentoService = {
    async criar(data: CriarEstabelecimento, usuarioId: string) {
  ...
  if (busca) {
    ...
    return { data: estabelecimentos, meta: { pagina, limite, total, totalPaginas: Math.ceil(total / limite) } }
  },
  ...
  async buscarPorId(id: string) {
    const estabelecimento = await prisma.estabelecimento.findUnique({
      ...
      include: {
          ...
          _count: { select: { administradores: true, pois: true, socorristas: true, codigosAcesso: true, redesWifi: true } },
        },
    })
  ...
  async atualizar(id: string, data: AtualizarEstabelecimento, usuarioId: string) {
